...
final class ProcessKqueue extends BaseEventProcessor<OsxProcess>
{
   ...
   // Arena for allocations that live as long as this processor.
   private final Arena arena;
   // Re-used in process() to avoid repeatedly allocating and destroying array of events.
   private final MemorySegment processEvents;
   private final MemorySegment waitTimeSpec;
   ...
   private ProcessKqueue(int lingerIterations)
   {
      ...
      arena = Arena.ofAuto();
      processEvents = Kevent.allocateArray(arena, NUM_KEVENTS);
      waitTimeSpec = TimeSpec.allocate(arena);

      // Listen for self-generated signal to tell processor to wake-up and handling pending stdin requests
      Kevent.EV_SET(Kevent.at(processEvents, 0), LibC.SIGUSR2, Kevent.EVFILT_SIGNAL, Kevent.EV_ADD | Kevent.EV_RECEIPT, 0, 0L, 0L);
   ...
   public void queueRead(OsxProcess process)
   {
      ...
      try (Arena eventArena = Arena.ofConfined()) {
         ...
         MemorySegment events = Kevent.allocateArray(eventArena, 4);
         // Listen for process exit (one-shot event)
         Kevent.EV_SET(Kevent.at(events, 0), pid, Kevent.EVFILT_PROC, Kevent.EV_ADD | Kevent.EV_RECEIPT | Kevent.EV_ONESHOT,
                       Kevent.NOTE_EXIT | Kevent.NOTE_EXITSTATUS | Kevent.NOTE_REAP, 0L, pid);
         // Listen for stdout and stderr data availability (events deleted automatically when file descriptors closed)
         Kevent.EV_SET(Kevent.at(events, 1), stdoutFd, Kevent.EVFILT_READ, Kevent.EV_ADD | Kevent.EV_RECEIPT, 0, 0L, pid);
         Kevent.EV_SET(Kevent.at(events, 2), stderrFd, Kevent.EVFILT_READ, Kevent.EV_ADD | Kevent.EV_RECEIPT, 0, 0L, pid);
         ...
         Kevent.EV_SET(Kevent.at(events, 3), stdinFd, Kevent.EVFILT_WRITE, Kevent.EV_ADD | Kevent.EV_DISABLE | Kevent.EV_RECEIPT, 0, 0L, pid);
   ...
   private void registerEvents(MemorySegment keventArray, int numEvents)
   {
      int ret = LibKevent.kevent(kqueue, keventArray, numEvents, keventArray, numEvents, MemorySegment.NULL);
      if (ret != numEvents) {
         throw new RuntimeException(String.format("Error %d registering events (ret=%d)", LibKevent.getLastError(), ret));
      }
      else {
         ...
         for (int i = 0; i < ret; i++) {
            int error = (int) Kevent.getData(Kevent.at(keventArray, i));
            if (error != 0) {
   ...
   public boolean process()
   {
      ...
      if (pidToProcessMap.isEmpty()) {
         ...
         TimeSpec.set(waitTimeSpec, LINGER_TIME_MS / 1000, TimeUnit.MILLISECONDS.toNanos(LINGER_TIME_MS % 1000));
         timeout = waitTimeSpec;
      }
      else {
         // At least one process is registered. There's no need to have a timeout.
         timeout = MemorySegment.NULL;
      }

      int nev = LibKevent.kevent(kqueue, MemorySegment.NULL, 0, processEvents, NUM_KEVENTS, timeout);
      if (nev == -1) {
      ...
      for (int i = 0; i < nev; i++) {
         processEvent(Kevent.at(processEvents, i));
      }
      return true;
   }
   ...
   private void processEvent(MemorySegment kevent)
   {
      int ident = (int) Kevent.getIdent(kevent);
      int filter = Kevent.getFilter(kevent);
      int udata = (int) Kevent.getUdata(kevent);
      ...
      {
         int available = (int) Kevent.getData(kevent);
         int flags = Kevent.getFlags(kevent);
         try {
         ...
         try {
            ...
            if (ident == stdinFd) {
               ...
               if (!userWantsMore) {
                  ...
                  try (Arena eventArena = Arena.ofConfined()) {
                     MemorySegment events = Kevent.allocateArray(eventArena, 1);
                     Kevent.EV_SET(Kevent.at(events, 0), stdinFd, Kevent.EVFILT_WRITE, Kevent.EV_DISABLE | Kevent.EV_RECEIPT, 0, 0L,
                                   osxProcess.getPid());
      ...
      else if ((Kevent.getFFlags(kevent) & Kevent.NOTE_EXIT) != 0) // process has exited System.gc()
      {
         cleanupProcess(osxProcess);
         int status = (int) Kevent.getData(kevent);
         if (WIFEXITED(status)) {
   ...
   private void checkWaitWrites()
   {
      ...
      if (!processes.isEmpty()) {
         ...
         try (Arena eventArena = Arena.ofConfined()) {
            MemorySegment kevents = Kevent.allocateArray(eventArena, processes.size());
            int numKevents = 0;
            for (int i = 0; i < processes.size(); i++) {
               ...
               try {
                  ...
                  if (fd != -1) {
                     Kevent.EV_SET(Kevent.at(kevents, numKevents), fd, Kevent.EVFILT_WRITE, Kevent.EV_ENABLE | Kevent.EV_RECEIPT, 0, 0L,
                                   process.getPid());
                     numKevents++;
                  }
               } finally {
   ...
   {
      LibC.waitpid(osxProcess.getPid(), new int[1], LibC.WNOHANG);
